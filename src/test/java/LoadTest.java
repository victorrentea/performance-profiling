...
public class LoadTest extends Simulation {
...

  {
    setUp(scenario(getClass().getSimpleName())
        ...
        // 1) Closed World (easier): N parallel threads firing requests in a loop
        //   => max N concurrent requests at any time
        .injectClosed(constantConcurrentUsers(43).during(ofSeconds(8))))

        // 2) Open World: N new requests / second => closer to a www userbase
        //   => higher latency => higher concurrency
